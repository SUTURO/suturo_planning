...
(defun init-tts()
  ;;init action client
  (roslisp:ros-info (init-interface) "init text to speech action client")
...
(defun init-nlg()
  ;; init nlg client for text generation
  (llif::init-nlg-action-client)
  (roslisp:ros-info (init-interface) "init natural language generation client")
)
