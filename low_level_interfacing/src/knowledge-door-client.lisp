...
(defun prolog-get-door-state(door-id)
  "Receives door ID `door-id'. Gives back the state of that door"
  (roslisp:ros-info (json-prolog-client)
                    "Getting the state for the door")
  (let* ((knowrob-name (format nil "~a~a" +hsr-rooms-prefix+ door-id))
    ...
    (cond
      ((eq raw-response 1) (roslisp:ros-warn (json-prolog-client)
                                             "Query didn't reach any solution."))
      (t (cdr (assoc '?Doorstate (cut:lazy-car raw-response)))))))
...
(defun prolog-get-all-door-states()
  ...
  (roslisp:ros-info (json-prolog-client)
                    "Getting all doors and their states")
  (let* ((raw-response
           (with-safe-prolog
...
(defun prolog-update-door-state(door-id angle)
  "Receives door ID `door-id' and angle `angle'. Updates the state of the door and the angle that the door was closed/opened at"
  (roslisp:ros-info (json-prolog-client)
                    "Updating the door state")
  (let* ((knowrob-name (format nil "~a~a" +hsr-rooms-prefix+ door-id))
         (raw-response (with-safe-prolog
                         (json-prolog:prolog-simple
...
(defun prolog-get-angle-to-open-door (door-id)
  "Receives door ID `door-id'. Returns the angle that is used to open the door"
  (let* ((knowrob-name (format nil "~a~a" +hsr-rooms-prefix+ door-id))
         (raw-response (with-safe-prolog
                         (json-prolog:prolog-simple
    ...
    (cond
      ...
      (t (cdr (car (cut:lazy-car raw-response)))))))
...
(defun prolog-perceiving-pose-of-door (door-id)
  "Receives door ID `door-id'. Returns the pose needed to perceive the door"
  (let* ((knowrob-name (format nil "~a~a" +hsr-rooms-prefix+ door-id))
         (raw-response (with-safe-prolog
                         (json-prolog:prolog-simple
    ...
    (cond
      ((eq raw-response 1) (roslisp:ros-warn (json-prolog-client)
                                             "Query get_door_state didn't reach any solution."))
      (t (list (cdr (assoc '?Pose (cut:lazy-car raw-response))))))))

;;@author Jan Schimpf
(defun prolog-get-doorhandle (door-id)
  "Receives door ID `door-id'. Returns the doorhandle knowrob name that is needed to open the door"
  (let* ((knowrob-name (format nil "~a~a" +hsr-rooms-prefix+ door-id))
         (raw-response (with-safe-prolog
                         (json-prolog:prolog-simple
...
(defun prolog-manipulating-pose-of-door(door-id)
  "Receives door ID `door-id'. Returns the pose from which we can manipulate the door"
  (let* ((knowrob-name (format nil "~a~a" +HSR-ROOMS-PREFIX+ door-id))
         (raw-response (with-safe-prolog
                         (json-prolog:prolog-simple
...
(defun prolog-shortest-path-between-rooms (start-room-id target-room-id)
  "Receives ID of the start room `start-room-id' and ID of the target room `target-room-id'. Returns door ids which are obstacles on the path frorm start room to target room"
...
(defun prolog-knowrob-name-to-urdf-link(door-id)
  "Receives door ID `door-id'. Returns the urdf link that has to do is linked to the knowrob name"
  (let* ((knowrob-name (format nil "~a~a" +HSR-ROOMS-PREFIX+ door-id))     
         (raw-response (with-safe-prolog
                         (json-prolog:prolog-simple
    ...
    (cond
      ((eq raw-response 1) (roslisp:ros-warn (json-prolog-client)
                                             "Query get_door_state didn't reach any solution."))
      (t (string-trim "'" (cdr (assoc '?UrdfName (cut:lazy-car raw-response))))))))
