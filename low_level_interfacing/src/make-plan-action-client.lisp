...
(defun get-make-plan-action-client ()
  "Returns the currently used make plan action client. If none yet exists, creates one."
  (or *make-plan-client*
      (init-make-plan-action-client)))

;; used in cleanup
(defun init-make-plan-action-client ()
  "Initializes the make-plan-action-client and makes sure it is connected to the action server."
  ...
  (roslisp:ros-info (make-plan-action)
                    "Make plan action client created"))
...
(defun make-plan-action-goal (start-pose object-frame-id goal-pose object-size gripper-mode action-mode)
  "Receives start pose `start-pose', object-frame ID `object-frame-id', goal pose `goal-pose', object size `object-size',  gripper mode `gripper-mode' and action mode `action-mode'. Creates the make-plan-action-goal."
...
(defun try-make-plan-action (start-pose object-frame-id goal-pose object-size gripper-mode action-mode)
  "Receives start pose `start-pose', object-frame ID `object-frame-id', goal pose `goal-pose', object size `object-size',  gripper mode `gripper-mode' and action mode `action-mode'."
