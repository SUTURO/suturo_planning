...
(defun save-init-map (mapMsg)
  "Receives message `mapMsg'. Saves the given `mapMsg' in the searchMap parameter."
  (when (not *searchMap*)
...
(defun create-map-message (map-message data-array)
  "Receives message `map-message' and array `data-array'. Returns a updated map-message using `data-array'."
...
(defun mark-position-visited (radius &optional position)
  "Receives radius `radius' and an optional position `position'. Marks `radius', which is a radius around the robot in the map as already searched."
  (roslisp:with-fields
      ((datas (data))
...
(defun find-biggest-unsearched-space (&optional debug)
  "Returns the position of the bottom left corner of the biggest area not searched yet and the size"
           ...
           (setf (aref copy i)
                 (+  
                  (min (aref copy (- i 1))
                       ...
                       (aref copy (- i width 1)))
                  1))
           (when (< max-size (aref copy i))                       
             (setf max-size (aref copy i))
             (setf bl-corner i)))
            (T (setf (aref copy i) 0))))
        ...
        (let* ((bl-coord (cl-tf:make-3d-vector
          ...
          (cond
           ...
           ;;TODO maybe change it from middle to smth else
             ...
             (roslisp::ros-info (find-biggest-unsearched-space)
                                "Square center: ~a"
                                square-center)
             square-center)
           (T
...
(defun publish-debug-square (pose-list)
  "Receives pose-list `pose-list'. Publishs a square marker, to visualise regions in rviz. `pose-list' contains the 4 corners of the square as a 3d Vector."
